...
async addCards(category: string,difficult: string) {
    ...
    for (let i = 0; i < finImages.length; i++) {
        ...
        (document.querySelector('.game-container') as HTMLElement).appendChild(new Card(finImages[i], this.cat).card);

        if(this.dif == '18'){
            (document.querySelector('.game-container') as HTMLElement).style.gridTemplateColumns = 'repeat(6,auto)';
