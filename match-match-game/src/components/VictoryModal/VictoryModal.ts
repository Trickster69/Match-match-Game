...
export class VictoryModal {
  ...
  CloseBtn: HTMLElement;
    constructor(score:string | number) {
        ...
        this.cancelBtn = (document.querySelector('.register-form__close-modal') as HTMLElement);
        this.formFeedback = document.querySelector('#register-form__form') as HTMLElement;
        ...
        this.emailInput = (document.querySelector('.register-form__email') as HTMLElement).querySelector('input'),
        ...
        this.showModal();
        this.closeModal();
        this.checkInputValue((document.querySelector('.first_name') as HTMLElement).querySelector('input'));
        this.checkInputValue((document.querySelector('.last_name') as HTMLElement).querySelector('input'));
        this.activeButton();
        this.indexedBd();
    ...
    closeModal() {
        ...
        (document.querySelector('.register-form__close-modal') as HTMLElement).addEventListener('click', () => {
          (document.querySelector('.overlay ') as HTMLElement).classList.remove('visible');
          (document.querySelector('.victory-modal') as HTMLElement).classList.remove('visible');
          (document.querySelector('body') as HTMLElement).classList.remove('victory');
        });

        (document.querySelector('.overlay') as HTMLElement).addEventListener('click', ()=> {
          (document.querySelector('.overlay ') as HTMLElement).classList.remove('visible');
          (document.querySelector('.victory-modal') as HTMLElement).classList.remove('visible');
          (document.querySelector('body') as HTMLElement).classList.remove('victory');
        })
    ...
    showModal() {
        (document.querySelector('.overlay') as HTMLElement).classList.add('visible');
        (document.querySelector('.victory-modal') as HTMLElement).classList.add('visible');
    ...
    checkInputValue(element:HTMLInputElement | null) {
        ...
        element.addEventListener('keyup', ()=> {
            ...
            if(!patternSymb.test(element.value) && (!regNumsOnly.test(element.value)) && (element.value.length > 0 && element.value.length < 30) &&(element.value.trim().length > 0)) {
        ...
        this.emailInput.addEventListener('keyup', ()=> {
          if(!this.emailInput) throw Error("check emailInput in checkInputValue VicoryModel class")
            if(emaliPattern.test(this.emailInput.value) && (element.value.length > 0 && element.value.length < 30)){
    ...
    activeButton() {
        for (let i = 0; i < this.modalFields.length; i++) {
            this.modalFields[i].addEventListener('keyup', ()=> {
                if([...this.modalFields].every((key => key.classList.contains('valid')))){
                    this.activeSendBtn();
                }else {
        ...
        if (!('indexedDB' in window)) {
            ...
            request.onsuccess = function() {
                ...
                (document.querySelector('form') as HTMLElement).addEventListener('submit',(e)=>{
                    e.preventDefault();
                    let name = (document.querySelectorAll('.register-form__input')[0] as HTMLInputElement).value;
                    let surname = (document.querySelectorAll('.register-form__input')[1] as HTMLInputElement).value;
                    let email = (document.querySelectorAll('.register-form__input')[2] as HTMLInputElement).value;
                    let score = (document.querySelector('#score') as HTMLElement).textContent;
