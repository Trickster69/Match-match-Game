  ...
  countDown!: NodeJS.Timeout;
    ...
    showCardsBeforeGame() {
        ...
        this.cardsArray.forEach(card => {
            (card.querySelector(".card__front") as HTMLElement).classList.remove('matched_bg');
    ...
    cardMatch(card1:HTMLElement, card2:HTMLElement) {
      if(!this.matchedCards) throw Error("check matchedCards in Game")
        ...
        (card1.querySelector(".card__front") as HTMLElement).classList.add('matched_bg');
        card2.classList.add('matched');
        (card2.querySelector(".card__front") as HTMLElement).classList.add('matched_bg');
    ...
    cardMisMatch(card1:HTMLElement, card2:HTMLElement) {
      // if(!this.MistakeCount) throw Error("check MistakeCount in Game")
        ...
        this.busy = true;
        (card1.querySelector(".card__front") as HTMLElement).classList.add('no_matched');
        (card2.querySelector(".card__front") as HTMLElement).classList.add('no_matched');

        setTimeout(()=> {
            (card1.querySelector(".card__front") as HTMLElement).classList.remove('no_matched');
            (card2.querySelector(".card__front") as HTMLElement).classList.remove('no_matched');
        },1500);
    ...
    getCardType(card:HTMLElement) {
        return (card.querySelector('.card__front') as HTMLElement).style.backgroundImage;
    }
    ...
    shuffleCards() {
        for(let i = this.cardsArray.length-1; i > 0; i--) {
            let randIndex:number = Math.floor(Math.random() * (i + 1));
